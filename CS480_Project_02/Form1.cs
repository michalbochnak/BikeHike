...
namespace BikeHike
{
  public partial class Form1 : Form
  {
      ...
      else
      {
        ...
        else if (rid <= -1000)
        {
          MessageBox.Show("Following bike is already rented: "
            + (-rid).ToString() + ".\nPick different bike.");
          LoadBikesForRentList();
          if (-rid == Convert.ToInt32(AllBikesListBox.SelectedItem))
            UpdateBikeInfo();
        }
        else
        {
    ...
    //
    // BikesForRentListBox_SelectedIndexChanged():
    //
    ...
    //
    // ExpDurationTextBox_TextChanged():
    //
