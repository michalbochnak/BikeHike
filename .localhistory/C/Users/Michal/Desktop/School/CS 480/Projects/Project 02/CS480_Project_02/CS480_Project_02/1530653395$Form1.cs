...
namespace BikeHike
{
  public partial class Form1 : Form
  {
    ...
    private void RentButton_Click(object sender, EventArgs e)
    {
      ...
      int cid = AllCustomersListBox.SelectedIndex;

      if (selectedBikes.Count == 0)
        MessageBox.Show("Select at least one bike.");
      else if (AllCustomersListBox.SelectedIndex == -1)
        MessageBox.Show("Select a customer.");
      else if (business.GetRentStatus(cid) == true)
      {
        business.RentBikes(selectedBikes, AllCustomersListBox.SelectedIndex);
      }
    }
  } // Form class
