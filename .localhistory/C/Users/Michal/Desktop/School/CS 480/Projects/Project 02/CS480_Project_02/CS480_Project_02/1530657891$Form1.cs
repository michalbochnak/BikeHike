...
namespace BikeHike
{
  public partial class Form1 : Form
  {
    ...
    private void RentButton_Click(object sender, EventArgs e)
    {
      ...
      else if (business.GetRentStatus(customers.ElementAt(index).CID.ToString()) == true)
        MessageBox.Show("Customer is already renting.");
      else if (ExpDurationTextBox.Text == "")
        MessageBox.Show("Enter duration.");
      else
      {
        business.RentBikes(selectedBikes, customers.ElementAt(index).CID, Convert.ToDecimal
          (ExpDurationTextBox.Text));
        MessageBox.Show(business.GetLastRentalId().ToString());
      }
    }
  } // Form class
