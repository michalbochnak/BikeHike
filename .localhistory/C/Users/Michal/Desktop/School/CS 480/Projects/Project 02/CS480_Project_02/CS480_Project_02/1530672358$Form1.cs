...
namespace BikeHike
{
  public partial class Form1 : Form
  {
    ...
    private void RentButton_Click(object sender, EventArgs e)
    {
      ...
      else if (business.GetCustRentStatus(cid.ToString()) == true)
        MessageBox.Show("Customer is already renting.");
      else if (ExpDurationTextBox.Text == "")
        MessageBox.Show("Enter duration.");
      else
      {
        MessageBox.Show("Rental ID: " + business.RentBikes(selectedBikes, cid, 
    ...
    private void ReturnButton_Click(object sender, EventArgs e)
    {
      ...
      if (index == -1)
        MessageBox.Show("Select a customer.");
      else if (business.GetCustRentStatus(cid.ToString()) == false)
        MessageBox.Show("This customer is not renting currently.");
      else
      {
        List<int> bikesToReturn = business.GetBikesToReturn(cid);
        ...
        MessageBox.Show("Total price: $" + business.ReturnBikes(cid));
        RentingTextBox.Text = "No";
        NumBikesTextBox.Text = "-";
        ExpReturnTextBox.Text = "-";
        BuildBikesForRentList();
